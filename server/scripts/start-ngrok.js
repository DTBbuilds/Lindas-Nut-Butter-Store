...
function startNgrok(port = 5000) {
  ...
  ngrokProcess.stdout.on('data', (data) => {
    ...
    if (match && match[1]) {
      ...
      // Ask user if they want to update configuration files
      rl.question('\nDo you want to update configuration files with this URL? (y/n): ', (answer) => {
        if (answer.toLowerCase() === 'y') {
          updateConfigFiles(ngrokUrl);
        } else {
          console.log('\n❌ Configuration not updated. You can manually update the URLs in:');
          console.log(`   - ${SERVER_CONFIG_PATH}`);
          console.log(`   - ${FRONTEND_CONFIG_PATH}`);
        }
      });
    }
  });
