...
const connectDB = async (retryCount = 0, maxRetries = 5) => {
  try {
    ...
    // Create indexes and initialize collections if needed
    await initializeDatabase();
    
    return mongoose.connection;
  } catch (err) {
    console.error(`MongoDB connection failed (attempt ${retryCount + 1}/${maxRetries + 1}):`, err.message);
...
const initializeDatabase = async () => {
  try {
    // Require the Admin model to ensure it's initialized
