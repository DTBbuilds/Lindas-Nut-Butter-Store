...
export const AdminAuthProvider = ({ children }) => {
  ...
  const checkAdminAuth = async (retryCount = 0, forceFresh = false) => {
    ...
    console.log(`Checking admin authentication status... (Attempt ${retryCount + 1}/${MAX_AUTH_RETRIES + 1})`);
    ...
    console.log(`Admin token found for email: ${adminEmail || 'unknown'}`);
    ...
    try {
      console.log(`Making auth verification request to ${API_URL}/api/auth/admin/me?minimal=true`);
      // Use minimal=true for faster verification without database lookup
      const response = await axios.get(`${API_URL}/api/auth/admin/me?minimal=true`, {
      ...
      if (response.data) {
        console.log('Valid admin found, setting admin state');
        // If response has user data, use it as admin
