import { API_CONFIG, getApiUrl, createQueryString, createRequestConfig } from '../config/api';
...
const responseInterceptor = async (response, url, options) => {
  ...
  // For all other endpoints, support both old and new response formats
  if (data) {
    // If the data is in the new format (has 'data' property)
...
const fetchWithRetry = async (url, options = {}, retries = API_CONFIG.MAX_RETRIES) => {
  ...
  try {
    ...
    // If the response is not ok and we should retry
    if (!response.ok && 
        retries > 0 && 
